...
export class ISLDiagnosticsProvider {
  ...
  provideDiagnostics(document: TextDocument): Diagnostic[] {
    ...
    const declaredBehaviors = new Set<string>();

    if (parsed) {
      for (const sym of parsed.symbols) {
        if (sym.kind === 'type') declaredTypes.add(sym.name);
        if (sym.kind === 'entity') declaredEntities.add(sym.name);
        if (sym.kind === 'behavior') declaredBehaviors.add(sym.name);
      }
    }
    ...
    for (let lineNum = 0; lineNum < lines.length; lineNum++) {
      ...
      if (currentBlock === 'behavior') {
        if (trimmed === 'input {') hasInput = true;
