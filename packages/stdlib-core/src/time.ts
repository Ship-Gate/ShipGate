...
export function isValidBusinessHours(hours: BusinessHours): boolean {
  if (!isValidTimezone(hours.timezone)) return false;
  if (hours.days.length === 0) return false;
  
  const startMs = hours.start.hour * 60 * 60 * 1000 + hours.start.minute * 60 * 1000 + hours.start.second * 1000;
  const endMs = hours.end.hour * 60 * 60 * 1000 + hours.end.minute * 60 * 1000 + hours.end.second * 1000;
...
export function isValidRecurringSchedule(schedule: RecurringSchedule): boolean {
  if (schedule.interval < 1 || schedule.interval > 365) return false;
  if (!isValidISODate(schedule.startDate)) return false;
  if (schedule.endDate && !isValidISODate(schedule.endDate)) return false;
  if (schedule.endDate && schedule.startDate > schedule.endDate) return false;
