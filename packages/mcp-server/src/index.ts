...
// ============================================================================
// AI Firewall — check_suggestion implementation
// ============================================================================
...
function checkSuggestion(args: {
  code_diff: string;
  truthpack_path?: string;
  file_path?: string;
}): FirewallResult {
  const { code_diff, file_path } = args;
  const violations: FirewallResult['violations'] = [];
  const suggestions: string[] = [];

  // Parse diff for route references
  const routePatterns = code_diff.match(/['"`](\/api\/[^'"`\s]+)['"`]/g) ?? [];
  const envPatterns = code_diff.match(/process\.env\[['"`](\w+)['"`]\]|process\.env\.(\w+)/g) ?? [];

  // Ghost route detection
  for (const route of routePatterns) {
    const routePath = route.replace(/['"`]/g, '');
    violations.push({
      rule: 'GHOST_ROUTE_CHECK',
      message: `Route reference ${routePath} — verify it exists in ISL API spec`,
      severity: 'warn',
    });
  }

  // Ghost env var detection
  for (const envMatch of envPatterns) {
    const varName = envMatch.replace(/process\.env\[['"`]|['"`]\]|process\.env\./g, '');
    violations.push({
      rule: 'GHOST_ENV_CHECK',
      message: `Env var ${varName} — verify it is declared in ISL config or .env.example`,
      severity: 'warn',
    });
  }
  ...
  const secretPatterns = [
    ...
    /(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{30,}/g,
  ];
  for (const pattern of secretPatterns) {
    if (pattern.test(code_diff)) {
      violations.push({
        rule: 'SECRET_HARDCODED',
        message: 'Potential hardcoded secret detected in diff',
        severity: 'critical',
      });
      suggestions.push('Move secrets to environment variables referenced via ISL config block');
    }
  }

  // Auth check detection
  if (code_diff.includes('app.get(') || code_diff.includes('app.post(') ||
      code_diff.includes('router.get(') || code_diff.includes('router.post(')) {
    const hasAuthMiddleware = code_diff.includes('auth') || code_diff.includes('verifyToken') ||
      code_diff.includes('isAuthenticated') || code_diff.includes('requireAuth');
    if (!hasAuthMiddleware) {
      violations.push({
        rule: 'AUTH_MISSING_CHECK',
        message: 'New route handler may lack authentication middleware',
        severity: 'warn',
      });
      suggestions.push('Add authentication middleware if this endpoint requires auth');
    }
  }

  // SQL injection detection
  if (/\$\{.*\}.*(?:SELECT|INSERT|UPDATE|DELETE|FROM|WHERE)/i.test(code_diff) ||
      /['"`]\s*\+\s*\w+\s*\+\s*['"`].*(?:SELECT|INSERT|UPDATE|DELETE)/i.test(code_diff)) {
    violations.push({
      rule: 'SQL_INJECTION',
      message: 'Potential SQL injection: raw string interpolation in query',
      severity: 'critical',
    });
    suggestions.push('Use parameterized queries or an ORM');
  }

  // Determine verdict
  const hasCritical = violations.some(v => v.severity === 'critical');
  const hasWarn = violations.some(v => v.severity === 'warn' || v.severity === 'high');

  const verdict: FirewallResult['verdict'] = hasCritical ? 'BLOCK'
    : hasWarn ? 'WARN'
    : 'ALLOW';

  return { verdict, violations, suggestions };
}
...
const UNIFIED_TOOL_SCHEMAS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'AI Firewall: Check a code suggestion/diff against Truthpack and policies. Returns ALLOW/BLOCK/WARN verdict with violation details.',
    inputSchema: {
      type: 'object',
      properties: {
        code_diff: { type: 'string', description: 'The code diff or suggestion to check' },
        truthpack_path: { type: 'string', description: 'Path to the Truthpack directory (optional, auto-detected)' },
        file_path: { type: 'string', description: 'Path of the file being modified (for context)' },
      },
      required: ['code_diff'],
    },
  },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'check_suggestion':
      return formatMCPResponse(
        checkSuggestion(args as { code_diff: string; truthpack_path?: string; file_path?: string }),
      );
