...
policyCommand
  .command('list')
  .description('List all available policy rules with severity, description, and remediation hints')
  .option('--category <cat>', 'Filter by category (auth, secrets, pii, cors, validation, etc.)')
  .option('--json', 'Output as JSON')
  .action(async (options) => {
    try {
      let manifest: { getPolicyManifest: () => Array<{ id: string; severity: string; description: string; remediation: string; languages: string[]; category: string }>;
        getRulesByCategory: (cat: string) => Array<{ id: string; severity: string; description: string; remediation: string; languages: string[]; category: string }>;
        getPolicyManifestJSON: () => object };

      try {
        manifest = await import('@isl-lang/policy-engine') as typeof manifest;
      } catch {
        // Fall back to direct path if the workspace package isn't linked
        manifest = await import('../../packages/policy-engine/src/policy-manifest.js') as typeof manifest;
      }

      if (options.json) {
        console.log(JSON.stringify(manifest.getPolicyManifestJSON(), null, 2));
        return;
      }

      const rules = options.category
        ? manifest.getRulesByCategory(options.category)
        : manifest.getPolicyManifest();

      console.log('');
      console.log(chalk.bold(`  ShipGate Policy Rules (${rules.length} total)`));
      console.log('');

      const bySeverity = new Map<string, typeof rules>();
      for (const rule of rules) {
        const list = bySeverity.get(rule.severity) ?? [];
        list.push(rule);
        bySeverity.set(rule.severity, list);
      }

      for (const [severity, sRules] of bySeverity) {
        const color = severity === 'critical' ? chalk.red
          : severity === 'high' ? chalk.yellow
          : severity === 'medium' ? chalk.cyan
          : chalk.gray;

        console.log(color.bold(`  ${severity.toUpperCase()} (${sRules.length}):`));
        for (const rule of sRules) {
          console.log(`    ${chalk.bold(rule.id)}: ${rule.description}`);
          console.log(chalk.gray(`      Fix: ${rule.remediation}`));
          console.log(chalk.gray(`      Languages: ${rule.languages.join(', ')}`));
        }
        console.log('');
      }
    } catch (err) {
      console.error(chalk.red(`Policy engine not available: ${err instanceof Error ? err.message : String(err)}`));
      process.exit(1);
    }
  });
...
program
  ...
  .option('--lang <language>', 'Target language: typescript (default), python, rust, go')
...
// ─────────────────────────────────────────────────────────────────────────────
// Open Command — Open most recent evidence report in browser
// ─────────────────────────────────────────────────────────────────────────────

program
  .command('open')
  .description('Open the most recent evidence HTML report in your default browser')
  .option('-d, --dir <path>', 'Evidence directory (default: .shipgate/evidence)')
  .action(async (options) => {
    try {
      const { openEvidence } = await import('./commands/evidence-html.js');
      const result = await openEvidence(options.dir);
      if (result.opened) {
        console.log(chalk.green(`Opened: ${result.path}`));
      } else if (result.path) {
        console.log(chalk.yellow(`Could not open browser. Report at: ${result.path}`));
      } else {
        console.log(chalk.yellow('No evidence reports found. Run `shipgate verify` first.'));
      }
    } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
// Scan Command — Scan any project for ISL specs + gate verdict
// ─────────────────────────────────────────────────────────────────────────────

program
  .command('scan [path]')
  .description('Scan a project: generate ISL specs, run truthpack, gate\n  Examples:\n    shipgate scan .\n    shipgate scan ./my-project --min-coverage 80')
  .option('--include <patterns>', 'File extensions to include (comma-separated, e.g. ts,py,go)')
  ...
  .action(async (targetPath: string | undefined, options) => {
    ...
    const result = await runProjectScan(scanPath, {
