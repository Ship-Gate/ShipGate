...
async function healLoop(
  ...
  while (iteration < maxIterations) {
    ...
    const healPrompt = `The following code violations were found after generating from an ISL spec.
Fix the violations while maintaining the behavioral contract.
...
For each file with violations, provide the corrected code as a JSON array:
[{ "path": "src/path/to/file.ts", "content": "// corrected content..." }]

Fix ONLY the violations. Do not add or remove functionality beyond what the spec requires.
...
Return ONLY the corrected ISL spec in a \`\`\`isl code block.`;
    ...
    stages.push({
      ...
      details: { fileCount: codeResult.files.size, tokens: codeResult.tokens },
    });
