...
describe('vibe --lang flag', () => {
  it('shows --lang in vibe help', () => {
    const result = exec('vibe --help');
    expect(result.exitCode).toBe(0);
...
describe('vibe pipeline unit tests', () => {
  it('ISL spec validation works on well-formed ISL', async () => {
    const vibeModule = await import('../src/commands/vibe.js');
    expect(vibeModule.vibe).toBeDefined();
  });

  it('SupportedLanguage types are exported', async () => {
    const vibeModule = await import('../src/commands/vibe.js');
    expect(vibeModule.SUPPORTED_LANGUAGES).toBeDefined();
    expect(vibeModule.SUPPORTED_LANGUAGES).toContain('typescript');
    expect(vibeModule.SUPPORTED_LANGUAGES).toContain('python');
    expect(vibeModule.SUPPORTED_LANGUAGES).toContain('rust');
    expect(vibeModule.SUPPORTED_LANGUAGES).toContain('go');
  });

  it('isValidLanguage validates language strings', async () => {
    const { isValidLanguage } = await import('../src/commands/vibe.js');
...
describe('codegen adapter', () => {
  it('getCodegenAdapter returns null for typescript (uses existing pipeline)', async () => {
    const { getCodegenAdapter } = await import('../src/commands/vibe/codegen-adapter.js');
    expect(getCodegenAdapter('typescript')).toBeNull();
  });

  it('getCodegenAdapter returns adapter for python', async () => {
    const { getCodegenAdapter } = await import('../src/commands/vibe/codegen-adapter.js');
    const adapter = getCodegenAdapter('python');
    expect(adapter).not.toBeNull();
    expect(adapter!.language).toBe('python');
  });

  it('getCodegenAdapter returns adapter for rust', async () => {
    const { getCodegenAdapter } = await import('../src/commands/vibe/codegen-adapter.js');
    const adapter = getCodegenAdapter('rust');
    expect(adapter).not.toBeNull();
    expect(adapter!.language).toBe('rust');
  });

  it('getCodegenAdapter returns adapter for go', async () => {
    const { getCodegenAdapter } = await import('../src/commands/vibe/codegen-adapter.js');
    const adapter = getCodegenAdapter('go');
    expect(adapter).not.toBeNull();
