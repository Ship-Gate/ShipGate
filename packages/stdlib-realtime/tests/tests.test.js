...
// Imports
// ============================================================================
import { ChannelType, PresenceStatus, MessageType, 
// Errors
RealtimeError, ChannelError, ChannelNotFoundError, MessageTooLargeError, ExponentialBackoff, FixedDelay, 
// Channels
ChannelManager, InMemoryChannelStore, InMemoryChannelSubscriptionStore, InMemoryMessageHistoryStore, DefaultChannelAuthorizer, 
// Presence
DefaultPresenceTracker, InMemoryPresenceStore, CodecFactory, HeartbeatFactory, } from '../src/index.js';
// ============================================================================
...
describe('Channel Management', () => {
    ...
    beforeEach(() => {
        ...
        channelManager = new ChannelManager(channelStore, subscriptionStore, historyStore, authorizer, undefined, mockClock);
    });
    it('should create and manage channels', async () => {
        const channel = await channelManager.createChannel('test-channel', ChannelType.BROADCAST, { maxMessageSize: 1024 });
    ...
    it('should publish messages to subscribers', async () => {
        ...
        const result = await channelManager.publish(channel.id, 'test-event', { message: 'hello' }, { connectionId: 'conn-1' });
...
describe('Presence Management', () => {
    ...
    it('should track user presence in channels', async () => {
        const presence = await presenceTracker.join('channel-1', 'user-1', 'conn-1', { status: PresenceStatus.ONLINE });
    ...
    it('should handle presence updates', async () => {
        ...
        const updated = await presenceTracker.updateStatus('channel-1', 'user-1', 'conn-1', PresenceStatus.AWAY);
...
describe('Integration Tests', () => {
    ...
    it('should handle complete flow: join -> subscribe -> publish -> leave', async () => {
        // Create channel
        const channel = await channelManager.createChannel('room-1', ChannelType.ROOM, { maxMessageSize: 1024 });
        // User joins presence
        const presence = await presenceTracker.join(channel.id, 'user-1', 'conn-1');
        expect(presence.status).toBe(PresenceStatus.ONLINE);
        // Subscribe to channel
        const subResult = await channelManager.subscribe('conn-1', channel.id);
        expect(subResult.channel.id).toBe(channel.id);
        // Publish message
        const pubResult = await channelManager.publish(channel.id, 'chat-message', { text: 'Hello everyone!' }, { connectionId: 'conn-1' });
        expect(pubResult.deliveredTo).toBe(1);
        // Update presence
        await presenceTracker.updateStatus(channel.id, 'user-1', 'conn-1', PresenceStatus.AWAY);
        // Leave channel
        await channelManager.unsubscribe('conn-1', channel.id);
    ...
    it('should handle multiple users in a channel', async () => {
        ...
        // Broadcast message
        const result = await channelManager.publish(channel.id, 'broadcast', { message: 'To all users' });
