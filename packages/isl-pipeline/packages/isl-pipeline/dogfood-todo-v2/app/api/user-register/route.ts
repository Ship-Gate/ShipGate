...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimitResult.success) {
    await auditAttempt({ success: false, reason: 'rate_limited', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
  ...
  try {
    ...
    if (!validationResult.success) {
      await auditAttempt({ success: false, reason: 'validation_failed', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
    ...
    await auditAttempt({ success: true, requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
    return NextResponse.json(result);
  } catch (error) {

    if (error instanceof EmailAlreadyExistsError) {
      await auditAttempt({ success: false, reason: 'emailAlreadyExists', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
      return NextResponse.json({ error: 'email is taken' }, { status: 400 });
    }

    if (error instanceof WeakPasswordError) {
      await auditAttempt({ success: false, reason: 'weakPassword', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
      return NextResponse.json({ error: 'password does not meet requirements' }, { status: 400 });
    }

    if (error instanceof PasswordMismatchError) {
      await auditAttempt({ success: false, reason: 'passwordMismatch', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
      return NextResponse.json({ error: 'passwords do not match' }, { status: 400 });
    }
    await auditAttempt({ success: false, reason: 'internal_error', requestId, action: 'UserRegister', timestamp: new Date().toISOString() });
