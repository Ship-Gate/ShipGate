...
// src/server.ts
import { DiagnosticSeverity as DiagnosticSeverity3 } from "@isl-lang/lsp-core";
var ISLServer = class {
  connection;
  documents;
  documentManager;
  ...
  setupHandlers() {
    ...
    this.connection.onRequest("isl/validate", (params) => {
      ...
      const errors = diagnostics.filter((d) => d.severity === DiagnosticSeverity3.Error).map((d) => d.message);
