...
import { 
  ...
  type XFailConfig,
  type XFailHarness,
} from '../../../test-fixtures/xfail-harness.ts';
...
describe('XFAIL Harness', () => {
  ...
  describe('Edge Cases', () => {
    it('should handle fixture paths with various formats', () => {
      const config: XFailConfig = {
        skip: [{ fixture: 'valid/test-unique.isl', reason: 'Test' }],
        xfail: [],
      };
      
      // Exact match
      expect(shouldSkip(config, 'valid/test-unique.isl')).toBeDefined();
      
      // Path ending with fixture
      expect(shouldSkip(config, 'test-fixtures/valid/test-unique.isl')).toBeDefined();
      
      // No match (different directory)
      expect(shouldSkip(config, 'invalid/other.isl')).toBeUndefined();
    });
