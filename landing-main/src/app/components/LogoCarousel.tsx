...
export default function LogoCarousel() {
  return (
    ...
    >
      <div className="relative w-full max-w-2xl overflow-hidden mb-4"
        style={{
      ...
      >
        ...
        >
          {[...LOGOS, ...LOGOS].map((logo, i) => (
            <div
              key={`${logo.title}-${i}`}
              className="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-xl backdrop-blur border border-white/10 shadow-sm hover:shadow-md transition-shadow shrink-0"
      ...
      <p className="text-sm text-white/60 uppercase tracking-wider">Works with your AI editor</p>
    </motion.div>
  );
}
